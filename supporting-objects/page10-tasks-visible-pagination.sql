...
--   Application:     12892
--   Name:            Task Center
--   Date and Time:   20:45 Tuesday June 2, 2026
...
'                and t.created_at >= add_months(systimestamp, -1)',
'           then ''Under 1 Month''',
'           when st.status_code in (''NOT_STARTED'', ''IN_PROGRESS'')',
'                and t.created_at < add_months(systimestamp, -1)',
'                and t.created_at >= add_months(systimestamp, -3)',
'           then ''1-3 Months''',
'           when st.status_code in (''NOT_STARTED'', ''IN_PROGRESS'')',
'                and t.created_at < add_months(systimestamp, -3)',
'           then ''Over 3 Months''',
...
'       case',
'           when v.overdue_yn = ''Y''',
'            and nvl(upper(v.status_code), ''~'') not in (''COMPLETED'', ''CLOSE'', ''CLOSED'')',
'           then ''Y''',
'           else ''N''',
'       end as overdue_yn,',
'       case',
...
' order by v.due_date desc nulls last, v.task_key'))
,p_ajax_enabled=>'Y'
